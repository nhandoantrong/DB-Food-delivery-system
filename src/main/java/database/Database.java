...
//    private static final String JDBC_DRIVER = "com.mysql.jdbc.Driver";
    ...
    private ArrayList<Menu> menuToArrayList(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            menus.add(new Menu(rs.getInt("Menu.ID"),
                               rs.getString("Menu.MenuStr"),
                               true));
        }
        return menus;
    }
    ...
    private ArrayList<Food> foodToArrayList(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            foods.add(new Food(rs.getInt("Food.ID"),
                               rs.getString("Food.FoodStr")));
        }
        return foods;
    }

    private ArrayList<Order> orderToArrayList(ResultSet rs) throws SQLException {
        rs.beforeFirst();
    ...
    private ArrayList<Material> materialToArrayList(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            materials.add(new Material(rs.getInt("Material.ID"),
                                       rs.getString("Material.MaterialStr"),
    ...
    private ArrayList<Customer> customerToArrayList(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            customers.add(new Customer(rs.getInt("Customer.ID"),
                                       rs.getString("Customer.Name"),
                                       rs.getInt("Customer.Tel"),
    ...
    public Menu getMenu(Integer id) {
        ArrayList<Menu> a = query("SELECT * FROM MENU WHERE ID = " + id,
                                  "menu");
        assert (a.size() == 1);
        return a.get(0);
    }
    ...
    public ArrayList getFoods(Material material) {
        return query("SELECT * FROM Food, needs " +
                     "WHERE needs.materialID = " + material.getId(),
                     "food");
    }

    public Food getFood(Integer id) {
        ArrayList<Food> a = query("SELECT * FROM Food WHERE ID = " + id,
                                  "food");
        assert (a.size() == 1);
        return a.get(0);
    }
